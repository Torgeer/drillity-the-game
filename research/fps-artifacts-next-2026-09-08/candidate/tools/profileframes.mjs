...
import { installLiveShimmerPairs, assessShimmerPairs } from './live-shimmer-pairs.mjs';
...
/** CDP/V8 signed cumulative timestamps, matching DevTools' paired ordering.
 * Raw sample/delta arrays are never edited, dropped, clamped or independently
 * sorted. CPU and Perfetto trace clock domains are not assumed equivalent.
 * Historical v1 verdicts remain unchanged; this is an explicit new protocol.
 */
export function assessLiveDiagnosticsV2(profile, traceEvents) {
  const faults = [], nodes = Array.isArray(profile?.nodes) ? Array.from(profile.nodes) : [];
  const samples = Array.isArray(profile?.samples) ? Array.from(profile.samples) : [];
  const deltas = Array.isArray(profile?.timeDeltas) ? Array.from(profile.timeDeltas) : [];
  const ids = new Set(nodes.map(n => n?.id)), negativeDeltas = [], timeline = [];
  if (!nodes.length || ids.size !== nodes.length || nodes.some(n => !Number.isSafeInteger(n?.id) || n.id < 1 || !n.callFrame)
      || !samples.length || samples.length !== deltas.length || samples.some(id => !Number.isSafeInteger(id) || id < 1 || !ids.has(id))
      || !Number.isSafeInteger(profile?.startTime) || !Number.isSafeInteger(profile?.endTime)
      || profile.startTime < 0 || profile.endTime <= profile.startTime) faults.push('CDP CPU profile shape/bounds invalid');
  let timestamp = profile?.startTime;
  for (let i = 0; i < deltas.length; i++) {
    const delta = deltas[i];
    if (!Number.isSafeInteger(delta)) { faults.push('CDP time delta is not a safe signed integer'); timestamp = NaN; }
    else timestamp += delta;
    if (!Number.isSafeInteger(timestamp) || timestamp < profile?.startTime || timestamp > profile?.endTime) faults.push('CDP cumulative sample timestamp outside profile bounds');
    if (Number.isSafeInteger(delta) && delta < 0) negativeDeltas.push({ index: i, deltaUs: delta, timestampUs: timestamp, nodeId: samples[i] });
    timeline.push({ index: i, timestamp, nodeId: samples[i] });
  }
  const ordered = [...timeline].sort((a, b) => a.timestamp - b.timestamp || a.index - b.index);
  if (!(ordered.at(-1)?.timestamp > profile?.startTime)) faults.push('CDP sample span empty');
  const events = Array.isArray(traceEvents) ? Array.from(traceEvents) : [];
  if (events.some(e => !e || typeof e !== 'object' || Array.isArray(e))) faults.push('Trace event shape invalid');
  const marks = name => events.filter(e => e?.name === name && String(e.cat).includes('blink.user_timing') && Number.isFinite(e.ts));
  const start = marks('drillity-live-start'), end = marks('drillity-live-end');
  if (!events.length || start.length !== 1 || end.length !== 1 || !(end[0]?.ts > start[0]?.ts)) faults.push('Live trace or correlation marks missing/invalid');
  return { protocol: 'cdp-signed-cumulative-v2', valid: faults.length === 0, faults: [...new Set(faults)],
    profileSamples: samples.length, traceEvents: events.length, negativeDeltas,
    signedTotalUs: timestamp - profile?.startTime, chronologicalFirstUs: ordered[0]?.timestamp ?? null, chronologicalLastUs: ordered.at(-1)?.timestamp ?? null,
    orderedSampleIndices: ordered.map(t => t.index),
    reorderedSamples: ordered.flatMap((t, index) => index === t.index ? [] : [{ rawIndex: t.index, chronologicalIndex: index }]),
    traceStartUs: start[0]?.ts ?? null, traceEndUs: end[0]?.ts ?? null,
    semantics: 'Signed deltas reconstruct cumulative sample timestamps; chronological order is a stable permutation of timestamp/sample pairs. Raw profile remains unchanged. CPU-profile and Perfetto trace clocks are independently validated, not assumed mapped for phase attribution.' };
}
...
export function installLiveHarness({ durationMs = 45000, instrument = true, vfxOrder = null } = {}) {
  ...
  const current = () => ({ completed, fatal, requestedDurationMs: durationMs, instrument,
    ...
    shimmerPairs: window.__SHIMMER_PAIRS?.current() || null,
  ...
  const cost = (name, ms) => {
    if (!frame || !instrument || window.__SHIMMER_PAIRS?.inMember) return;
  ...
  async function finish(reason = null) {
    ...
    try { await window.__SHIMMER_PAIRS?.finish(); } catch (e) { fatal ||= String(e); }
  ...
  try {
    ...
    c.renderer.render = function (...args) {
      ...
      try { return oldRender.apply(this, args); }
        ...
        if (!fatal && window.__SHIMMER_PAIRS) {
          const began = performance.now();
          try { window.__SHIMMER_PAIRS.run(frame.frame); } catch (e) { laterFailure(e); }
          finally { frame.shimmerDiagnosticMs = performance.now() - began; }
        }
      }
    };
...
async function run() {
  ...
  const shimmerPairs = Number(flag('--live-shimmer-pairs', '0'));
  if (shimmerPairs && (!live || !instrumentLive || vfxOrder || !Number.isSafeInteger(shimmerPairs) || shimmerPairs < 8 || shimmerPairs > 60)) throw Error('Invalid live shimmer pair options');
  if (!Number.isSafeInteger(shimmerPairs) || (args.includes('--live-shimmer-pairs') && !shimmerPairs)) throw Error('Invalid live shimmer pair count');
  ...
  const manifest = () => {
    ...
    for (const p of ['index.html', 'vite.config.js', '.env', '.env.local', '.env.development', '.env.development.local', 'package.json', 'package-lock.json', 'tools/profileframes.mjs', 'tools/live-shimmer-pairs.mjs', 'node_modules/three/package.json', 'node_modules/three/build/three.module.js']) if (existsSync(resolve(root, p))) files.push(resolve(root, p));
  ...
  const report = { version: shimmerPairs ? 4 : live ? 3 : 2, mode: shimmerPairs ? 'live-shimmer' : live ? 'live' : 'frozen', started: new Date().toISOString(), rig, camera, requestedFrames: live ? null : n, repeats: live ? null : repeats,
  ...
  try {
    server = await createServer({ root, cacheDir: resolve(root, '.vite-profile-cache'), server: { host: '127.0.0.1', port, strictPort: true, hmr: false } });
    await server.listen();
    ...
    if (live) {
      ...
      report.liveOptions = { liveMs, seed, instrumentLive, vfxOrder, shimmerPairs };
    ...
    if (live) {
      ...
      report.browserProtocol = await cdp.send('Browser.getVersion');
      const traceEvents = [];
      let liveCpuProfile;
      ...
      try {
        if (shimmerPairs) report.shimmerInstall = await page.evaluate(installLiveShimmerPairs, { requestedPairs: shimmerPairs });
        ...
        if (shimmerPairs) report.live.shimmerAssessment = assessShimmerPairs(report.live.shimmerPairs);
        save();
      } finally {
        ...
        if (instrumentLive) {
          ...
          report.liveDiagnostics = shimmerPairs ? assessLiveDiagnosticsV2(liveCpuProfile, traceEvents) : assessLiveDiagnostics(liveCpuProfile, traceEvents);
        }
        try { report.cleanup.live = await page.evaluate(() => window.__LIVE_PROFILE?.cleanup()); }
        catch (e) { report.cleanup.live = { error: String(e) }; }
        try { report.cleanup.shimmer = await page.evaluate(() => window.__SHIMMER_PAIRS?.cleanup()); }
        catch (e) { report.cleanup.shimmer = { error: String(e) }; }
      ...
      if (shimmerPairs && !report.live?.shimmerAssessment?.valid) throw Error(`Live shimmer pairs rejected: ${report.live?.shimmerAssessment?.faults.join('; ') || 'no complete record'}`);
      ...
      if (report.cleanup.liveCpu || report.cleanup.liveTrace || report.cleanup.liveCdp || report.cleanup.live?.error || report.cleanup.shimmer?.error) throw Error('Live capture cleanup failed; see cleanup details');
    ...
    if (page && !page.isClosed()) { try { report.cleanup.page = await page.evaluate(async () => {
      ...
      const shimmer = window.__SHIMMER_PAIRS ? await window.__SHIMMER_PAIRS.cleanup() : null;
      window.__LIVE_SETUP?.sheet?.close();
      return { live, frozen, shimmer, installed: !!(live || frozen || shimmer) };
