...
const baseline = readFileSync(resolve(flag('baseline-asset', resolve(root, 'public/models/sites/quarry-bench.glb'))));
...
try {
  ...
  for (const variant of ['baseline','candidate']) {
    ...
    const materialProbe = await page.evaluate(() => {
      const c=window.__DRILLITY, rows=[];
      c.terrain.root.getObjectByName('site:quarry-bench').traverse(o=>{
        if(!o.isMesh)return;
        for(const m of (Array.isArray(o.material)?o.material:[o.material])) {
          const sets=[...new Set(c.assets._sets.values())].filter(s=>s.map===m.map);
          rows.push({mesh:o.name,kind:o.userData.siteKinds,name:m.name,map:!!m.map,normalMap:!!m.normalMap,
            setReady:sets.map(s=>({family:s.family,ready:s.ready,res:s.res}))});
        }
      });
      return {stats:c.assets.stats(),materials:rows};
    });
    report.textureReadiness ??= [];
    report.textureReadiness.push({variant,before:materialProbe});
    await page.waitForFunction(()=>{
      const s=window.__DRILLITY.assets.stats();
      if(s.failed>0)throw Error('Texture generation failed: '+s.failed);
      return s.sets>0&&s.pending===0&&s.setsReady===s.sets;
    },null,{timeout:60000});
    const settled=await page.evaluate(()=>window.__DRILLITY.assets.stats());
    assert.equal(settled.failed,0);assert.equal(settled.pending,0);assert.equal(settled.setsReady,settled.sets);
    report.textureReadiness.at(-1).after=settled;
